...
const generateRandomColor = () => {
  ...
  const lightness = Math.floor(Math.random() * 50); // Lightness value to ensure contrast with white text
  return `hsl(${hue}, ${saturation}%, ${lightness}%)`;
};

const generateSegmentColors = (segments: string[]) => {
  const segColors = new Set();
  while (segColors.size < segments.length) {
    let color;
    do {
      color = generateRandomColor();
    } while (segColors.has(color));
    segColors.add(color);
  }
  return Array.from(segColors) as string[];
};

const WheelComponent = ({
  segments = ["Segment 1", "Segment 2", "Segment 3"],
  segColors = generateSegmentColors(segments),
  winningSegment,
  onFinished,
  primaryColor = 'black',
  contrastColor = 'white',
  ...
  const spin = () => {
    ...
    if (timerHandle === 0) {
      ...
      maxSpeed = Math.PI / segments.length + 50;
      frames = 0;
  ...
  const drawWheel = () => {
    ...
    ctx.fill();
    ctx.font = 'bold 1.75em ' + fontFamily;
    ctx.fillStyle = contrastColor;
    ctx.textAlign = 'center';
  ...
  return (
    <div id={wheelId.current} className='centered flex'>
      <canvas
        id={canvasId.current}
        width={dimension}
        height={dimension}
